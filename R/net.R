#' Render the net for a capsid model.
#'
...
#' \dontrun{
#' # Adenovirus
#' net(5, 5, R = 10, aesthetics = list(line = list(size = 1)))
...
#' @export
netOutput <- function(outputId, width = "100%", height = "400px") {
  htmlwidgets::shinyWidgetOutput(outputId, "net", width, height, package = "rdemocapsid")
}

#' @rdname rdemocapsid-shiny
#' @export
renderNet <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  htmlwidgets::shinyRenderWidget(expr, netOutput, env, quoted = TRUE)
}
